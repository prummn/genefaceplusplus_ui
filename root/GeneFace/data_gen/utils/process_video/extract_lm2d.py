...
if __name__ == '__main__':
    ...
    # === 修改开始 ===
    # 如果 worker 数量为 1，直接在主线程运行，避免 fork 死锁
    if args.num_workers <= 1:
        for i, args_item in enumerate(tqdm.tqdm(job_args, desc=f"Root {args.process_id}: Single Process")):
            # 手动解包参数调用函数
            res = extract_landmark_job(*args_item)
            if res is False:
                fail_cnt += 1
            print(
